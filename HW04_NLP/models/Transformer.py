...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2, dim_feedforward=2048, dim_k=96, dim_v=96, dim_q=96, max_length=43):
        ...
        #                               END OF YOUR CODE                             #
        ##############################################################################
        self.word_emb_layer = nn.Embedding(input_size, self.word_embedding_dim)
        self.pos_emb_layer  = nn.Embedding(self.max_length, self.word_embedding_dim)
        ##############################################################################
        ...
        ##############################################################################
        self.flayer = nn.Linear(self.hidden_dim, self.output_size)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
    ...
    def embed(self, inputs):
        ...
        #############################################################################
        N, T = inputs.size()
        embeddings = self.word_emb_layer(inputs) + self.pos_emb_layer(torch.arange(T))
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return embeddings
    ...
    def multi_head_attention(self, inputs):
        ...
        # pdb.set_trace()
        # HEAD 1
        at1 = torch.bmm(q1, k1.permute(0, 2, 1)) / (self.dim_v ** 0.5)
        at1 = torch.softmax(at1, dim=2)
        at1 = torch.bmm(at1, v1)

        # HEAD 2
        at2 = torch.bmm(q2, k2.permute(0, 2, 1)) / (self.dim_v ** 0.5)
        at2 = torch.softmax(at2, dim=2)
        at2 = torch.bmm(at2, v2)

        # concatenate
        at_net = torch.cat((at1, at2), dim=2)
        at_net = self.attention_head_projection(at_net)
        at_net += inputs
        outputs = self.norm_mh(at_net)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return outputs
