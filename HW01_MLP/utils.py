...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    #    batch size                                                             #
    #############################################################################
    def dummyFunc():        # used for random suffling in same order on two lists
        random.seed(seed)
        return random.random()

    if shuffle:
        random.shuffle(batched_data, dummyFunc)
        random.shuffle(batched_label, dummyFunc)

    num_total = len(data)
    quotient = num_total // batch_size
    remainder = num_total % batch_size  # will be the size of last batch if non_zero
    for i in range(0, quotient):
        batched_data.append(np.array(data[i*batch_size : (i + 1)*batch_size]))   # append image batch
        batched_label.append(np.array(label[i*batch_size : (i + 1)*batch_size])) # append label batch
    
    if remainder != 0:
        batched_data.append(np.array(data[num_total - remainder :]))      # append the final image batch
        batched_label.append(np.array(label[num_total - remainder :]))    # append the final label batch
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.plot(range(len(valid_loss_history)), valid_loss_history, label='valid')
    plt.legend()
    ...
    plt.xlabel("Epochs")
    plt.ylabel("acc")
    plt.plot(range(len(train_acc_history)), train_acc_history, label='train')
    plt.plot(range(len(valid_acc_history)), valid_acc_history, label='valid')
    plt.legend()
