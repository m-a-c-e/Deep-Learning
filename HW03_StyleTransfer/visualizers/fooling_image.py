...
class FoolingImage:
    def make_fooling_image(self, X, target_y, model):
        ...
        for it in range(max_iter):
            ...
            ##############################################################################
            pred_scores = model(X_fooling_var)  # raw scores
            pred_class  = torch.argmax(torch.exp(pred_scores) / torch.sum(torch.exp(pred_scores)))
            if pred_class.detach().numpy() == target_y:
                # model fooled!
                break
            
            loss = pred_scores[0][target_y]
            loss.backward()
            X_fooling += learning_rate * X_fooling_var.grad / torch.norm(X_fooling_var.grad)
