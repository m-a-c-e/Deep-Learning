...
def guided_gradcam_captum():
    ...
    ##############################################################################
    ggc = GuidedGradCam(model, layer=conv_module)
    attr_ggc = ggc.attribute(X_tensor, target=y_tensor)
    plt = visualize_attr_maps("Grads Captum", X, y, class_names, [attr_ggc], ['Guided GradCam'])
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return plt
...
def layer_vis_captum():
    ...
    ##############################################################################
    # get the conductance and gradcam attributes for the specific layer

    layer_cond = LayerConductance(model, layer)         
    layer_gcam = LayerGradCam(model, layer)             

    # cond = layer_cond.attribute(inputs=X_tensor, target=y_tensor)
    # N x 512 x 13 x 13
    attr_layer_cond = layer_cond.attribute(inputs=X_tensor, target=y_tensor)
    # N x 1 x 13 x 13
    attr_layer_gcam = layer_gcam.attribute(X_tensor, y_tensor, relu_attributions=True)

    attr_layer_cond = torch.mean(attr_layer_cond, dim=1, keepdim=True)

    # visualize using captum_utils
    plt = visualize_attr_maps(
            "Layer Vis",
            X,
            y,
            class_names,
            [attr_layer_cond, attr_layer_gcam],
            ["Conductance", "GradCam"],
            attr_preprocess=lambda attr: attr.permute(1, 2, 0).detach().numpy()[:, :, 0]
            )
